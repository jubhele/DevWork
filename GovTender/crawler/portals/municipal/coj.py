"""
City of Johannesburg (CoJ) tender crawler — joburg.org.za
Static HTML listing pages — uses BeautifulSoup (no Playwright needed).
Geographic scope: municipal:COJ
"""
...
_BASE_URL = "https://www.joburg.org.za"
_TENDER_URL = f"{_BASE_URL}/tenders"
_PORTAL_NAME = "municipal_coj"
_GEO = "municipal:COJ"
_ENTITY = "City of Johannesburg Metropolitan Municipality"

normaliser = Normaliser()


class COJCrawler(BaseCrawler):
        ...
        async with httpx.AsyncClient(
            ...
            while True:
                ...
                try:
                    ...
                    log_debug("CRAWL_HTTP_ERROR", {"portal": _PORTAL_NAME, "url": url, "error": str(exc)})
                    break
...
def _parse_listing(html: str) -> list[dict]:
    ...
    rows = []
    for row in soup.select("table tr, .tender-row, article.tender"):
        ...
        href = link["href"]
        rows.append({
            "ref_number": _text(cols[0]) if cols else link.get_text(strip=True),
            "title": _text(cols[1]) if len(cols) > 1 else link.get_text(strip=True),
            "closing_date_raw": _text(cols[2]) if len(cols) > 2 else "",
...
def _parse_detail(html: str) -> dict:
    soup = BeautifulSoup(html, "lxml")
    result: dict = {}
    body = soup.select_one(".tender-body, .field-body, article .content, .page-content")
    if body:
        result["description"] = body.get_text(separator="\n", strip=True)[:3000]
    for label in soup.select(".field-label, th"):
...
@app.task(name="crawler.portals.municipal.coj.run")
def run() -> dict:
    async def _run() -> int:
        ...
        count = 0
        async with COJCrawler() as crawler:
