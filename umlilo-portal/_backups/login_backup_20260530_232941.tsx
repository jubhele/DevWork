...
function LoginForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true)
    try {
      const res = await auth.login(username, password, parseInt(captcha.answer, 10))
      if (res.success) {
        router.push(next)
      } else {
      ...
      if (msg === 'Unauthenticated' || msg.includes('400') || msg.includes('Invalid security')) {
        setError('Incorrect credentials or security answer.')
      } else {
