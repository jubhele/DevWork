...
- The live dashboard bridge can return KPI data under `kpi`, not only `data`; normalize both shapes and handle `overdue_inv` as the overdue invoice field.
- Finance pages should not trust `total`; quotes and invoices may need totals derived from subtotal/tax fields or normalized by the API bridge.
- Removing legacy `portal.js` from the Next layout eliminates the `Unexpected token '<'` and missing DOM target console failures on portal routes.
- `/admin/audit` is the route currently linked and tested for the audit view.
- If an `.env` role fails login while other roles pass and audit logs show `LOGIN_FAIL`, treat it as a backend account/credential issue until the account is reset or mapped.
