...
async function fetchRecordData(entityType: EntityType, entityRef: string) {
  ...
  return {
    ...
    attachments: fileResponse.ok && fileBody.success ? fileBody.attachments ?? fileBody.data?.attachments ?? [] : [],
  } as { updates: TrackerUpdate[]; attachments: Attachment[] }
}

export default function TrackerRecordPanel({ entityType, entityRef, createdAt, startAt, endAt, dueAt, canEdit }: {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded border border-steel-dark bg-navy p-6 shadow-sm">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-4">
          <div><h2 className="font-display text-2xl text-bone-paper">Schedule</h2><p className="text-sm text-ash">Dates and times for this record</p></div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <label className="text-xs uppercase tracking-[0.15em] text-ash">Created<span className="mt-2 block rounded border border-steel-dark bg-charcoal px-3 py-2.5 text-sm normal-case tracking-normal text-bone-paper">{displayDateTime(createdAt)}</span></label>
          {(['start_at', 'end_at', 'due_at'] as const).map(field => (
            <label key={field} className="text-xs uppercase tracking-[0.15em] text-ash">{field === 'start_at' ? 'Start' : field === 'end_at' ? 'End' : 'Due'}
              <input type="datetime-local" value={schedule[field]} disabled={!canEdit} onChange={event => setSchedule(current => ({ ...current, [field]: event.target.value }))} className="mt-2 w-full rounded border border-steel-dark bg-navy px-3 py-2 text-sm normal-case tracking-normal text-bone-paper disabled:bg-charcoal" />
            </label>
          ))}
        </div>
      </section>
      ...
      <section className="rounded border border-steel-dark bg-navy p-6 shadow-sm">
        <div className="mb-5"><h2 className="font-display text-2xl text-bone-paper">Description Records</h2><p className="text-sm text-ash">Each entry is separate and every edit preserves a revision.</p></div>
        <div className="space-y-4">
          {updates.map((update, index) => (
            <article key={update.id} className="rounded border border-steel-dark p-4">
              ...
              <input aria-label={`Label for description ${index + 1}`} value={update.label} disabled={!canEdit} onChange={event => setUpdates(current => current.map(item => item.id === update.id ? { ...item, label: event.target.value } : item))} className="mb-3 w-full rounded border border-steel-dark bg-navy px-3 py-2 text-sm font-semibold text-bone-paper" />
              <textarea aria-label={`Description ${index + 1}`} value={update.content} disabled={!canEdit} rows={4} onChange={event => setUpdates(current => current.map(item => item.id === update.id ? { ...item, content: event.target.value } : item))} className="w-full rounded border border-steel-dark bg-navy px-3 py-2 text-sm text-bone-paper" />
        ...
        {canEdit && <form onSubmit={addUpdate} className="mt-5 grid gap-3 rounded bg-charcoal p-4"><input name="label" required maxLength={120} placeholder="Label, e.g. Client feedback" className="rounded border border-steel-dark bg-navy px-3 py-2 text-sm text-bone-paper" /><textarea name="content" required maxLength={10000} rows={4} placeholder="Enter a new description or update" className="rounded border border-steel-dark bg-navy px-3 py-2 text-sm text-bone-paper" /><div className="text-right"><button className="rounded bg-fire-orange px-4 py-2 text-xs font-semibold uppercase tracking-[0.16em] text-white">Add description</button></div></form>}
      </section>

      <section className="rounded border border-steel-dark bg-navy p-6 shadow-sm">
        <div className="mb-5"><h2 className="font-display text-2xl text-bone-paper">Files</h2><p className="text-sm text-ash">Upload documents or images and open them from this record.</p></div>
        ...
        <div className="divide-y divide-steel-dark rounded border border-steel-dark">
          {attachments.map(file => <div key={file.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 text-sm"><div><p className="font-medium text-bone-paper">{file.original_name}</p><p className="text-xs text-ash">{Math.ceil(file.file_size / 1024)} KB | {file.uploaded_by} | {displayDateTime(file.created_at)}</p></div><a target="_blank" rel="noreferrer" href={`${API_BASE}/files.php?action=view&id=${file.id}`} className="text-xs font-semibold uppercase tracking-[0.15em] text-fire-orange">View</a></div>)}
