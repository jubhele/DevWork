...
export type PowerBISurface = 'dashboard' | 'invoices'
...
function getSurfaceConfig(surface: PowerBISurface): SurfaceConfig {
  if (surface === 'invoices') {
    return {
      ...
      reportId: process.env.POWERBI_INVOICES_REPORT_ID,
    }
  }
...
export function listMissingPowerBIEnv(surface: PowerBISurface): string[] {
  ...
  if (!reportId) {
    missing.push(surface === 'invoices' ? 'POWERBI_INVOICES_REPORT_ID' : 'POWERBI_DASHBOARD_REPORT_ID')
  }
...
export function getPowerBIRolesForPortalRole(role: string): string[] {
  ...
  if (role === 'manager') return ['Portal_Manager']
  if (role === 'senior_tech' || role === 'tech' || role === 'operator') return ['Portal_Operations']
