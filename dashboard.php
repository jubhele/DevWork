...
require_once __DIR__ . '/../includes/scope_policy.php';
...
$engagement_ids = allowed_engagement_ids($scope_ctx);
$scope_placeholders = $engagement_ids
    ? implode(',', array_fill(0, count($engagement_ids), '?'))
    : 'NULL';
$scope_sql = static fn(string $alias): string =>
    "{$alias}.company_client_id IN ({$scope_placeholders})";
$scope_c = $scope_sql('c');
$scope_q = $scope_sql('q');
$scope_i = $scope_sql('i');
$scope_s = $scope_sql('s');
$scope_invoice_table = $scope_sql('bf_invoices');
...
// KPI counts
$open_callouts    = db_row("SELECT COUNT(*) AS n FROM bf_callouts c WHERE status IN ('Open','In Progress') AND {$scope_c}", $engagement_ids)['n'] ?? 0;
$urgent_callouts  = db_row("SELECT COUNT(*) AS n FROM bf_callouts c WHERE status IN ('Open','In Progress') AND priority IN ('Urgent','Emergency') AND {$scope_c}", $engagement_ids)['n'] ?? 0;
$pending_quotes   = db_row("SELECT COUNT(*) AS n FROM bf_quotes q WHERE status IN ('Draft','Sent','Pending Approval') AND {$scope_q}", $engagement_ids)['n'] ?? 0;
$overdue_invoices = db_row("SELECT COUNT(*) AS n FROM bf_invoices i WHERE status IN ('Sent','Overdue') AND due_date < CURDATE() AND {$scope_i}", $engagement_ids)['n'] ?? 0;
$pending_statements = can('finance.statement')
    ? (db_row("SELECT COUNT(*) AS n FROM bf_statements s WHERE status = 'pending_approval' AND {$scope_s}", $engagement_ids)['n'] ?? 0)
    : 0;
$active_clients   = db_row(
    "SELECT COUNT(DISTINCT c.id) AS n
     FROM bf_clients c
     JOIN bf_company_clients cc ON cc.client_id = c.id
     WHERE c.is_active = 1 AND cc.id IN ({$scope_placeholders})",
    $engagement_ids
)['n'] ?? 0;

// Safety score: average score across active, approved safety files (null if none exist)
$ss_row      = db_row("SELECT AVG(score) AS avg FROM bf_safety_files s WHERE status = 'Approved' AND is_active = 1 AND {$scope_s}", $engagement_ids);
    ...
    $row = db_row(
        "SELECT COUNT(*) AS invoice_count, COALESCE(SUM(amount),0) AS amount
          ...
          WHERE status != 'Cancelled'
            ...
            AND {$scope_invoice_table}",
        array_merge([$cursor->format('Y'), $cursor->format('n')], $engagement_ids)
    );
    $invoice_run_months[] = [
...
$outstanding = (float)(db_row(
    "SELECT COALESCE(SUM(amount),0) AS total
     FROM bf_invoices i
     WHERE status IN ('Sent','Overdue') AND {$scope_i}",
    $engagement_ids
)['total'] ?? 0);
$net_cash = (float)(db_row(
    "SELECT COALESCE(SUM(t.credit-t.debit),0) AS total
     ...
     WHERE t.trans_date BETWEEN ? AND ?
       AND EXISTS (
         SELECT 1 FROM bf_callouts c
         WHERE c.ref_id = t.callout_ref AND {$scope_c}
       )",
    array_merge([$period_start, $period_end], $engagement_ids)
)['total'] ?? 0);
$quote_pipeline = (float)(db_row(
    "SELECT COALESCE(SUM(total_amount),0) AS total FROM bf_quotes q
      WHERE status IN ('Draft','Sent','Pending Approval') AND quote_date BETWEEN ? AND ?
        AND {$scope_q}",
    array_merge([$period_start, $period_end], $engagement_ids)
)['total'] ?? 0);

$cash_total = static function (string $start, string $end) use ($scope_c, $engagement_ids): float {
    return (float)(db_row(
        "SELECT COALESCE(SUM(t.credit),0) AS total FROM bf_transactions t
          WHERE category = 'Invoice Payment' AND trans_date BETWEEN ? AND ?
            AND EXISTS (
              SELECT 1 FROM bf_callouts c
              WHERE c.ref_id = t.callout_ref AND {$scope_c}
            )",
        array_merge([$start, $end], $engagement_ids)
    )['total'] ?? 0);
};
...
$recent_callouts = db_select(
    "SELECT ref_id, client_name, service, status, priority, callout_date
     FROM bf_callouts c
     WHERE {$scope_c}
     ORDER BY callout_date DESC LIMIT 5",
    $engagement_ids
);
...
if (can('callout.view')) {
    ...
    $callout_where .= " AND {$scope_c}";
    $callout_params = array_merge($callout_params, $engagement_ids);
    ...
    $due_soon = array_merge($due_soon, db_select(
        "SELECT 'Invoice' AS record_type, i.ref_id, i.client_name AS record_title,
          ...
          WHERE i.status IN ('Draft','Sent')
            ...
            AND {$scope_i}",
        array_merge([$due_end], $engagement_ids)
    ));
}
if (can('quote.view')) {
    $due_soon = array_merge($due_soon, db_select(
...
if (can('security.users') || can('security.audit')) {
    $usage = db_select(
        "SELECT u.username, u.name, u.last_login,
          ...
          WHERE u.active = 1
            AND u.username != 'blackfm6w9f9_izilo'
