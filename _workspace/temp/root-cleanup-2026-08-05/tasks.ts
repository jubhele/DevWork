...
import { automaticDeadlineForCreation } from './deadlines'
...
export async function createTask(
  ...
  const dueAt = data.dueAt
    ? new Date(data.dueAt)
    : data.dueDate
      ? new Date(`${data.dueDate}T17:00:00+02:00`)
      : await automaticDeadlineForCreation()
...
export async function updateTask(
  ...
  if ('dueAt' in data) {
    const dueAt = data.dueAt ? new Date(data.dueAt) : await automaticDeadlineForCreation()
    update.dueAt = dueAt
    update.dueDate = new Date(dueAt)
  }
