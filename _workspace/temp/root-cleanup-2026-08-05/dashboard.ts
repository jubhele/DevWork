...
import { scopeInPredicate, scopedClientIds } from '@/lib/scope'
...
function scopedTransactionPredicate(ctx: ScopeContext) {
  if (!ctx.allowedEngagementIds.length) return sql`1 = 0`
  return sql`EXISTS (
    SELECT 1 FROM bf_callouts scoped_callout
    WHERE scoped_callout.ref_id = ${bfTransactions.calloutRef}
      AND scoped_callout.company_client_id IN (${sql.join(ctx.allowedEngagementIds.map((id) => sql`${id}`), sql`, `)})
  )`
}

async function collectedBetween(start: string, end: string, ctx: ScopeContext): Promise<number> {
  const [row] = await db
    ...
    .where(and(
      ...
      scopedTransactionPredicate(ctx),
    ))
  return Number(row?.total ?? 0)
}

export async function getDashboardData(user: DashboardUser, ctx: ScopeContext): Promise<DashboardData> {
  const now = new Date()
  ...
  const clientIds = await scopedClientIds(ctx)
  let openTasks = 0
  let urgentTasks = 0
  let tasksDueToday = 0
  ...
  const kpiResults = await Promise.allSettled([
    db.select({ n: count() }).from(bfCallouts).where(and(inArray(bfCallouts.status, ['Open', 'In Progress']), scopeInPredicate(bfCallouts.companyClientId, ctx))),
    db.select({ n: count() }).from(bfCallouts).where(and(inArray(bfCallouts.status, ['Open', 'In Progress']), inArray(bfCallouts.priority, ['Urgent', 'Emergency']), scopeInPredicate(bfCallouts.companyClientId, ctx))),
    db.select({ n: count() }).from(bfQuotes).where(and(inArray(bfQuotes.status, ['Draft', 'Sent', 'Pending Approval']), scopeInPredicate(bfQuotes.companyClientId, ctx))),
    db.select({ n: count() }).from(bfInvoices).where(and(inArray(bfInvoices.status, ['Sent', 'Overdue']), sql`${bfInvoices.dueDate} < ${today}`, scopeInPredicate(bfInvoices.companyClientId, ctx))),
    db.select({ n: count() }).from(bfClients).where(and(eq(bfClients.isActive, 1), clientIds.length ? inArray(bfClients.id, clientIds) : eq(bfClients.id, -1))),
    db.select({ n: count() }).from(bfStatements).where(and(eq(bfStatements.status, 'pending_approval'), scopeInPredicate(bfStatements.companyClientId, ctx))),
  ])
  ...
  const invoiceRunRate: DashboardRunRateMonth[] = await Promise.all(monthDates.map(async month => {
    ...
    const [row] = await db.select({
      ...
      scopeInPredicate(bfInvoices.companyClientId, ctx),
    ))
    return {
  ...
  const [outstandingRow, movementRow, quotePipelineRow] = await Promise.all([
    db.select({ total: sql<string>`COALESCE(SUM(${bfInvoices.amount}), 0)` })
      .from(bfInvoices)
      .where(and(inArray(bfInvoices.status, ['Sent', 'Overdue']), scopeInPredicate(bfInvoices.companyClientId, ctx))),
    db.select({ total: sql<string>`COALESCE(SUM(${bfTransactions.credit} - ${bfTransactions.debit}), 0)` })
      .from(bfTransactions)
      .where(and(sql`${bfTransactions.transDate} >= ${periodStart}`, sql`${bfTransactions.transDate} <= ${periodEnd}`, scopedTransactionPredicate(ctx))),
    db.select({ total: sql<string>`COALESCE(SUM(${bfQuotes.totalAmount}), 0)` })
      .from(bfQuotes)
      .where(and(inArray(bfQuotes.status, ['Draft', 'Sent', 'Pending Approval']), sql`${bfQuotes.quoteDate} >= ${periodStart}`, sql`${bfQuotes.quoteDate} <= ${periodEnd}`, scopeInPredicate(bfQuotes.companyClientId, ctx))),
  ])
  ...
  const [mtd, previousMtd, qtd, previousQtd, ytd, previousYtd] = await Promise.all([
    collectedBetween(monthStart, today, ctx),
    collectedBetween(previousMonthStart, previousToday, ctx),
    collectedBetween(quarterStart, today, ctx),
    collectedBetween(previousQuarterStart, previousToday, ctx),
    collectedBetween(yearStart, today, ctx),
    collectedBetween(previousYearStart, previousToday, ctx),
  ])
  ...
  if (canView(user, 'callout.view')) {
    ...
    conditions.push(scopeInPredicate(bfCallouts.companyClientId, ctx))
  ...
  if (canView(user, 'invoice.view')) {
    const rows = await db.select({ refId: bfInvoices.refId, title: bfInvoices.clientName, dueDate: bfInvoices.dueDate })
      .from(bfInvoices)
      .where(and(inArray(bfInvoices.status, ['Draft', 'Sent']), sql`${bfInvoices.dueDate} BETWEEN ${today} AND ${dueEnd}`, scopeInPredicate(bfInvoices.companyClientId, ctx)))
  ...
  if (canView(user, 'quote.view')) {
    const rows = await db.select({ refId: bfQuotes.refId, title: bfQuotes.clientName, assignee: bfQuotes.submittedBy, dueDate: bfQuotes.validUntil })
      .from(bfQuotes)
      .where(and(inArray(bfQuotes.status, ['Draft', 'Sent', 'Pending Approval']), sql`${bfQuotes.validUntil} BETWEEN ${today} AND ${dueEnd}`, scopeInPredicate(bfQuotes.companyClientId, ctx)))
  ...
  try {
    const [row] = await db.select({ average: sql<string | null>`AVG(${bfSafetyFiles.score})` })
      .from(bfSafetyFiles)
      .where(and(eq(bfSafetyFiles.status, 'Approved'), eq(bfSafetyFiles.isActive, 1), scopeInPredicate(bfSafetyFiles.companyClientId, ctx)))
  ...
  try {
    const rows = await db.select({
      ...
      .where(scopeInPredicate(bfCallouts.companyClientId, ctx))
