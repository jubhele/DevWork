...
export const bfUsers = mysqlTable('bf_users', {
  ...
  accessVersion: int('access_version', { unsigned: true }).default(1).notNull(),
}, (t) => [
...
// ─────────────────────────────────────────────────────────────────────────────
// RLS: company/client engagement scope (docs/row-level-security-company-client-
// isolation-plan.md §8; mirrors BlackFire Portal/includes/scope_policy.php).
// These tables/columns were added to the live local-dev schema by
// install/rls_company_client_isolation_20260725.sql — this section brings the
// Next.js Drizzle schema into parity with it (Phase 6 gap fix).
// ─────────────────────────────────────────────────────────────────────────────

export const bfCompanyClients = mysqlTable('bf_company_clients', {
  id: int('id', { unsigned: true }).autoincrement().primaryKey(),
  companyProfileId: int('company_profile_id', { unsigned: true }).notNull(),
  clientId: int('client_id', { unsigned: true }).notNull(),
  relationshipStatus: mysqlEnum('relationship_status', ['planned', 'active', 'transitioning', 'suspended', 'ended']).notNull(),
  validFrom: date('valid_from'),
  validTo: date('valid_to'),
  safetyRequired: tinyint('safety_required').default(1).notNull(),
  reportingEnabled: tinyint('reporting_enabled').default(1).notNull(),
  isPrimaryForClient: tinyint('is_primary_for_client').default(0).notNull(),
  ...
  uniqueIndex('uq_company_client').on(t.companyProfileId, t.clientId),
  index('idx_company_client_active').on(t.companyProfileId, t.relationshipStatus, t.clientId),
])

export const bfUserCompanyMemberships = mysqlTable('bf_user_company_memberships', {
  userId: int('user_id', { unsigned: true }).notNull(),
  companyProfileId: int('company_profile_id', { unsigned: true }).notNull(),
  clientAccessMode: mysqlEnum('client_access_mode', ['assigned', 'all_company_clients']).default('assigned').notNull(),
  isPrimary: tinyint('is_primary').default(0).notNull(),
  validFrom: datetime('valid_from').default(sql`CURRENT_TIMESTAMP`).notNull(),
  validTo: datetime('valid_to'),
  createdByUserId: int('created_by_user_id', { unsigned: true }).notNull(),
  createdAt: datetime('created_at').default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (t) => [
  primaryKey({ columns: [t.userId, t.companyProfileId] }),
  index('idx_membership_company').on(t.companyProfileId, t.userId),
])

export const bfUserClientAssignments = mysqlTable('bf_user_client_assignments', {
  userId: int('user_id', { unsigned: true }).notNull(),
  companyClientId: int('company_client_id', { unsigned: true }).notNull(),
  validFrom: datetime('valid_from').default(sql`CURRENT_TIMESTAMP`).notNull(),
  validTo: datetime('valid_to'),
  createdByUserId: int('created_by_user_id', { unsigned: true }).notNull(),
  createdAt: datetime('created_at').default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (t) => [
  primaryKey({ columns: [t.userId, t.companyClientId] }),
  index('idx_assignment_engagement').on(t.companyClientId, t.userId),
])

export const bfCrossCompanyOverrideContexts = mysqlTable('bf_cross_company_override_contexts', {
  id: bigint('id', { unsigned: true, mode: 'number' }).autoincrement().primaryKey(),
  actorUserId: int('actor_user_id', { unsigned: true }).notNull(),
  reason: varchar('reason', { length: 500 }).notNull(),
  companyProfileIds: varchar('company_profile_ids', { length: 200 }).notNull(),
  ...
  index('idx_override_actor').on(t.actorUserId),
])
...
export const bfCallouts = mysqlTable('bf_callouts', {
  ...
  companyClientId: int('company_client_id', { unsigned: true }),
}, (t) => [
  ...
  index('idx_co_company_client').on(t.companyClientId),
])
...
export const bfBusinessHolidays = mysqlTable('bf_business_holidays', {
...
export const bfQuotes = mysqlTable('bf_quotes', {
  ...
  status: mysqlEnum('status', ['Draft', 'Sent', 'Approved', 'Rejected', 'Pending Approval', 'Expired', 'Converted', 'Reversed']).default('Draft').notNull(),
  ...
  index('idx_q_company_client').on(t.companyClientId),
])
...
export const bfInvoices = mysqlTable('bf_invoices', {
  ...
  dueDateIsManual: tinyint('due_date_is_manual').default(0).notNull(),
  ...
  index('idx_inv_company_client').on(t.companyClientId),
])
...
export const bfDocumentReversalRequests = mysqlTable('bf_document_reversal_requests', {
  id: int('id', { unsigned: true }).autoincrement().primaryKey(),
  hostCompanyId: int('host_company_id', { unsigned: true }).default(1).notNull(),
  calloutId: int('callout_id', { unsigned: true }).notNull(),
  calloutRef: varchar('callout_ref', { length: 30 }).notNull(),
  quoteId: int('quote_id', { unsigned: true }).notNull(),
  quoteRef: varchar('quote_ref', { length: 30 }).notNull(),
  invoiceId: int('invoice_id', { unsigned: true }),
  invoiceRef: varchar('invoice_ref', { length: 30 }),
  reason: varchar('reason', { length: 1000 }).notNull(),
  status: mysqlEnum('status', ['pending', 'approved', 'rejected']).default('pending').notNull(),
  requestedByUserId: int('requested_by_user_id', { unsigned: true }).notNull(),
  requestedAt: datetime('requested_at').notNull(),
  decidedByUserId: int('decided_by_user_id', { unsigned: true }),
  decidedAt: datetime('decided_at'),
  decisionNotes: varchar('decision_notes', { length: 1000 }),
}, (t) => [
  index('idx_reversal_invoice_status').on(t.invoiceId, t.status),
  index('idx_reversal_callout').on(t.calloutId, t.requestedAt),
])
...
export const bfStatements = mysqlTable('bf_statements', {
  ...
  index('idx_stmt_company_client').on(t.companyClientId),
])
...
export const bfSafetyFiles = mysqlTable('bf_safety_files', {
  ...
  index('idx_safety_company_client').on(t.companyClientId),
])
