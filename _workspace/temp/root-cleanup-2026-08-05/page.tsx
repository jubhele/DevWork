...
import { buildScopeContext } from '@/lib/scope'
...
export default async function DashboardPage() {
  ...
  const scope = user ? await buildScopeContext(user) : null
  const dashboard = user && scope ? await getDashboardData(user, scope) : null
  const [activeCompany] = scope?.activeCompanyId
    ? await db
      .select({ name: schema.bfCompanyProfiles.displayName })
      .from(schema.bfCompanyProfiles)
      .where(eq(schema.bfCompanyProfiles.id, scope.activeCompanyId))
      .limit(1)
    : []
  const activeCompanyName = activeCompany?.name ?? 'Active company'
  ...
  return (
    <ReportFrame title="Dashboard" eyebrow={`${activeCompanyName} · ${user?.role === 'sysadmin' ? 'Sysadmin View' : 'Admin View'}`}>
      <section className="grid gap-5 rounded-[2px] border border-[#ccc] border-l-fire-orange bg-white p-5 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          ...
          <h2 className="mt-2 font-display text-3xl text-ink-text sm:text-4xl">{activeCompanyName} health check</h2>
