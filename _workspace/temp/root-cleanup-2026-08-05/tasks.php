...
if ($method === 'POST') {
    ...
    if (!$due_at && valid_date($body['due_date'] ?? null)) $due_at = $body['due_date'] . ' 17:00:00';
    if (!$due_at) $due_at = automatic_tracker_deadline();
    $due_date = substr($due_at, 0, 10);
...
if ($method === 'PUT') {
    ...
    if (array_key_exists('due_date', $body)) {
        $replacement_due_at = valid_date($body['due_date'] ?? null)
            ? $body['due_date'] . ' 17:00:00'
            : automatic_tracker_deadline();
        $set[]    = 'due_date = ?';
        $params[] = substr($replacement_due_at, 0, 10);
        $set[]    = 'due_at = ?';
        $params[] = $replacement_due_at;
        $changed[] = 'due_date';
        $changed[] = 'due_at';
    }
    foreach (['start_at', 'end_at', 'due_at'] as $field) {
        if (array_key_exists($field, $body)) {
            ...
            if ($field === 'due_at' && $value === null) $value = automatic_tracker_deadline();
            $set[] = "{$field} = ?";
            $params[] = $value;
            $changed[] = $field;
            if ($field === 'due_at') {
                $set[] = 'due_date = ?';
                $params[] = substr($value, 0, 10);
            }
        }
    }
