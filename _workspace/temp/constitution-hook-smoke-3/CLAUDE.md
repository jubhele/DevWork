...
**Mirror**: After writing or updating a session log, copy it under `G:\My Drive\JS\Agentic AI\sessions\<project-slug>\` with `.tbl.bk` appended. Preserve project ownership; never flatten every project's logs into one mirror folder.
...
Any file generated to investigate, test, or experiment goes in `<project-root>\temp\`.
This includes:
...
Genuinely cross-project scratch work goes in `c:\DevWork\_workspace\temp\`. Never use a shared
root `temp/` as an unowned dumping ground. Temp folders are gitignored and reviewed at session end;
stale does not mean safe to delete, and cleanup requires an explicit retention policy or user approval.

---

## 7c. Work Artifact Organization

All work artifacts belong below their owning project root, not loose at the workspace root.
...
Each project keeps project-owned `sessions/`, `artifacts/`, `archive/`, `temp/`, `logs/`, `_backups/`,
and an `ARTIFACT_INDEX.md`. `c:\DevWork\WORKSPACE_INDEX.md` is the root discovery map and is refreshed
at session end when project summaries change. Workspace-wide artifacts use `c:\DevWork\_workspace\`
with the same taxonomy. Ambiguous legacy items go to `_workspace\index\unresolved\`; never guess
ownership, overwrite collisions, or delete originals during indexing.

Never create loose artifacts at a project or workspace root. Constitutions, provider configuration,
repository manifests, workspace indexes, and minimum runtime entry points are control-plane exceptions.
Full migration, hashing, concurrency, privacy, and index-update constitution:
