...
Reconfirmed again: 2026-07-27, user asked to verify clickable-card work was applied — confirmed live in portal.js (data-filter-key/data-filter-value on Open Tasks/Open Callouts KPI cards), no code change needed, still c:\DevWork\BlackFire.
Reconfirmed again: 2026-07-28, user screenshotted "Due in 7 Days" and "Overdue Invoices" attention cards and correctly pointed out they weren't clickable — real gap in Phase 1, fixed. Still c:\DevWork\BlackFire.
...
- Two of the four ("Outstanding", "Quote Pipeline") represent multi-status sums per their own subtitles/queries (`status IN ('Sent','Overdue')` and `status IN ('Draft','Sent','Pending Approval')` respectively, matching `dashboard.php`'s own SQL) — single-value dropdown filtering couldn't express this, so extended `renderInvoices`/`renderQuotes` to accept a comma-separated status list (`filter.split(',')` + `.includes()`), backward-compatible with existing single-value dropdown usage. Added matching multi-value `<option>`s to `#inv-filter`/`#qte-filter` in `portal.php` ("Outstanding (Sent + Overdue)", "Pipeline (Draft + Sent + Pending)") so the dropdown UI stays honest about what's actually filtered, rather than silently showing "All" while the list is filtered underneath.
- "Net Cash Movement" navigates to `p-transactions` unfiltered (it's a summed transactions figure, not a single-entity status). "Invoiced YTD" navigates to `p-invoices` unfiltered (period-based sum, not a status filter).
- Rewrote the `amountCards` render template (was plain `<div>`s, no click handling at all) to match the `countCards`/`attentionCards` button pattern with `data-filter-key`/`data-filter-value` support.
- Discovered and fixed the same `renderQuotes` DOM-desync issue found earlier in `renderInvoices`: `renders['p-quotes']` always called `renderQuotes('')` ignoring the dropdown's actual value. Added `applyPendingQuoteFilter()` and explicit `#qte-filter` value passthrough, mirroring the `p-invoices` fix.
- QA (gstack browse, `dev-only/qa_browse_login.sh bf_manager`): "Outstanding" → navigated to Invoices, filter correctly showed "Sent,Overdue", 3 records shown, confirmed all 3 are Sent/Overdue (2 Sent, 1 Overdue). "Quote Pipeline" → navigated to Quotes, filter correctly showed the pipeline option, 2 records shown, confirmed one Sent one Pending Approval. "Net Cash Movement" → navigated to Transactions cleanly. "Invoiced YTD" → navigated to Invoices cleanly. No console errors on any click. Screenshot: `dev-only/qa-screenshots/phase1-amount-cards-clickable.png`.
- Backed up `portal.js`/`portal.php` before this edit round per §7a.

## Phase 1 follow-up fix — attentionCards not clickable — CLOSED
- User caught a real gap via screenshot: the `attentionCards` block (dashboard "Urgent/Due/Overdue" row) had its own separate button template that never emitted `data-filter-key`/`data-filter-value` at all — Phase 1 only fixed the `countCards` template. "Urgent Tasks"/"Urgent Callouts" navigated but without filters (correct, since urgency isn't a status value); "Due in 7 Days" and "Overdue Invoices" had no `page` at all, so weren't buttons — just static divs.
- Fixed the `attentionCards` render template (~portal.js:4015-4017) to emit `data-filter-key`/`data-filter-value` when present, matching `countCards`.
- Tagged "Overdue Invoices" with `page:'p-invoices', filterKey:'status', filterValue:'Overdue'`.
- Discovered `renderInvoices(search, filter)` doesn't read `#inv-filter`'s DOM value itself (unlike `renderCallouts`) — it only uses whatever `filter` argument is passed by the caller, and `renders['p-invoices']` always called it with `''`. Fixed by adding `applyPendingInvoiceFilter()` (same pattern as Phase 1/2) plus explicitly reading `#inv-filter`'s value into both `renderInvoices()` calls in `renders['p-invoices']`.
- Deliberately left "Due in 7 Days" non-clickable: it's an intentional cross-entity union (tasks+callouts+invoices+quotes per `dashboard.php`'s `due_soon` query) with no single target page — making it clickable would require a new combined view, out of scope for this fix.
- QA (gstack browse, via `dev-only/qa_browse_login.sh bf_manager` — manager role, has invoice.view): clicked "Overdue Invoices" KPI card on dashboard, confirmed navigation to `p-invoices` with `#inv-filter` set to "Overdue", and the visible list correctly showed exactly 1 record (`INV-AI20260626`, confirmed status="Overdue" in the rendered card) matching the KPI count. Screenshot: `dev-only/qa-screenshots/phase1-overdue-invoices-fix.png`.
